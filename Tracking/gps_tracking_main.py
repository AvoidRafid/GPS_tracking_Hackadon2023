...
ingolstadt_map = folium.Map(location=[48.766549705649446, 11.43147624757401], zoom_start=17, tiles='CartoDB positron')

# Define the coordinates for the Bus 20 route in Ingolstadt
route_coordinates = [
        [48.766549705649446, 11.43147624757401],
        ...
        [48.76133865341721, 11.426338017679988],  # Add more coordinates for the longer route...
...
heavy_traffic = [[[48.76594999229388, 11.43170615596545],
                  [48.7657533166793, 11.4316837772289080],
                  [48.76564514476295, 11.431683777228908],
                  [48.764425736133845, 11.432228326484713]],
                 [[48.741885051436306, 11.437442104902052],
...
medium_traffic = [[[48.7522463878005, 11.432074998961319],
                   [48.75133035632122, 11.432488287532463],  # stop
                   [48.751046267173805, 11.43258501467965],
                   [48.75052307393989, 11.432817976616379] ], 
                   [[48.75936791243698, 11.43082035370531],
...
stations_original = [
    [48.766549705649446, 11.43147624757401], #start
    [48.763678890517355, 11.431875631542546],
    [48.75428710664237, 11.43116048819216],  # stop
    [48.76191032444984, 11.4256604956094880],
    [48.75133035632122, 11.432488287532463],  # stop
...
bus_icon = folium.features.CustomIcon('bus.png', icon_size=(30, 30))
...
# Introduce a sleep time for heavy traffic zones
sleep_on_heavy_traffic = 2  # Adjust the sleep time as needed
sleep_on_medium_traffic = 1
...
for coords in route_coordinates:
    ...
    # Check if the current coordinates are in any heavy traffic zones
    in_heavy_traffic = any(
        any(
            coord[0] == coords[0] and coord[1] == coords[1] for coord in zone
        ) for zone in heavy_traffic
    )

    # Check if the current coordinates are in any medium traffic zones
    in_medium_traffic = any(
        any(
            coord[0] == coords[0] and coord[1] == coords[1] for coord in zone
        ) for zone in medium_traffic
    )

    # If the current coordinates are in heavy traffic, sleep longer
    if in_medium_traffic:
        time.sleep(sleep_on_medium_traffic)
    else:
        time.sleep(0.4)  # Sleep time for regular movement


    # If the current coordinates are in heavy traffic, sleep longer
    if in_heavy_traffic:
        time.sleep(sleep_on_heavy_traffic)
    else:
        time.sleep(0.4)  # Sleep time for regular movement
